...
class ReportMakerController extends CI_Controller
{
	...
	{
		$this->load->view("ReportMaker/reportTemplateBuilder", array("title" => "Report View", 'template_id' => $template_id));
	}

	public function reportMakerList()
	{
		$this->load->view("ReportMaker/reportMakerList", array("title" => "Report list View"));
	}
	...
	public function GetReportView()
	{
		...
		$mbData = $this->db
			->select(array("*"))
			->where(array('company_id' => $company_id, "status" => 1, "type" => $type))
			->order_by('id', 'desc')
			->get("report_maker_master")->result();
	...
	public function reportMakerByMonth()
	{
		...
		$this->load->view("ReportMaker/reportMakerByMonth", array("title" => "Report list View", "id" => $id, "templateName" => $templateName));
	}

	public function createReportMonthPdf()
	{
		if (!is_null($this->input->post('month')) && $this->input->post('month') != "" && !is_null($this->input->post('year')) && $this->input->post('year') != "") {
			...
			if ($resultObject->totalCount > 0) {
				$html = $resultObject->data->template_body;
				...
				$response['Form'] = $formulaArray;

				foreach ($formulaArray as $fvalue) {
					//$fvalue= str_replace(array(" ",",",";","'"), '', $fvalue);
					$string2 = strip_tags($fvalue);
			...
			foreach ($TablematchArray as $t_value) {
				switch ($t_value) {
					...
					case '#T_PL_1001':
						...
						break;
					case '#G_BS_1001':
						$graph = $this->getBalanceSheetGraph(1);
						array_push($graphArray, $graph);
						$html = str_replace($t_value, $graph->data, $html);
						break;
					case '#G_PL_1001':
						$graph = $this->getBalanceSheetGraph(1);
						array_push($graphArray, $graph);
			...
			$html = str_replace('<!-- my page break -->', '<br style="page-break-before: always">', $html);
			$response['status'] = 200;
			$response['body'] = $html;
			$response['graphArray'] = $graphArray;
		} else {
			$response['status'] = 201;
	...
	// function getGLBalanceQueryDataReport($month,$year,$column,$account_number,$type,$attr){

	//  	$gl_ac_number = $this->db->select('main_gl_number')->where('group_id',$gl_ac)->get('main_account_setup_master')->result();
	//  	 $gl_acs = implode(', ', $tags);
	// 	if ($type == 1) {
	// 		$table_name = 'consolidate_report_all_data_ind';
	// 	}elseif ($type == 2) {
	// 		$table_name = 'consolidate_report_all_data_us';
	// 	}else if ($type == 3) {
	// 		$table_name = 'consolidate_report_all_data_ifrs';
	// 	}

	// 	$data = $this->db->select('sum('.$column.') as total')->where('account_number IN('.$gl_acs.')')->get($table_name)->row();
	// 	return $data;
	// }
...
//			 print_r($this->db->last_query());exit();
	...
	public function uploadTemplateData()
	{
		...
		if (!is_null($this->input->post('templateName')) && !is_null($this->input->post('type'))) {
			...
			if ($template_id != 0) {
				...
				$resultObject = $this->Master_Model->_update('report_maker_master', $data, $where);
				...
				$resultObject = $this->Master_Model->_insert('report_maker_master', $data);
				if ($resultObject->status == true) {
					$response['status'] = 200;
	...
	{
		if (!is_null($this->input->post('template_id'))) {
			$template_id = $this->input->post('template_id');
			$resultObject = $this->Master_Model->_select('report_maker_master', array('id' => $template_id), "*", true);
			if ($resultObject->totalCount > 0) {
				$response['status'] = 200;
...
//		$docx->createDocx($fileName);
	...
	public function getDerivedGLData()
	{
		if (!is_null($this->input->post('type'))) {
			$company_id = $this->session->userdata('company_id');
			$type = $this->input->post('type');
			if ($type == 3) {
				...
				$table = 'derived_account_setup';
			}
			$resultObject = $this->Master_Model->_select($table, array('company_id' => $company_id), array('derived_account_gl', 'detail'), false);
			if ($resultObject->totalCount > 0) {
				$options = '';
				// $options='<option selected disabled>Select Gl ACCOUNT</option>';
				foreach ($resultObject->data as $key => $value) {
					...
					$options .= '<tr>
        					<td>' . $value->detail . '</td>
        					<td><span id="da_t' . $value->derived_account_gl . '">#DF_' . $value->derived_account_gl . '@</span><button class="btn btn-sm btn-link" onclick="copyText(\'da_t' . $value->derived_account_gl . '\')"><i class="fa fa-copy"></i></button></td>';
