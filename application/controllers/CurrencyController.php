...
//		$getDataBranch=$this->Master_Model->order_by_data($select="*,(select default_currency_rate from branch_master b where b.id=branch_id) as branch_name",$where=array(),$table="currency_conversion_master",$order_by="id",$key="asc");
	...
	{
		$mbData = $this->db->query('select id,gl_ac,exchange_rate,year,month,(select name from branch_master where id=spr.branch_id )as subsidiary from special_exchange_rate spr order by id desc')->result();
		$month_array = $this->Master_Model->getQuarter();
		$tableRows = array();
		if (count($mbData) > 0) {
			$i=1;
	...
	{
		$mbData = $this->db->query('select id,gl_ac,exchange_rate,year,month,(select name from branch_master where id=spr.branch_id )as subsidiary,type from special_additionGL_rate spr order by id desc')->result();
		$month_array = $this->Master_Model->getQuarter();
		$tableRows = array();
		if (count($mbData) > 0) {
			$i=1;
	...
	{
		$mbData = $this->db->query('select id,gl_ac,rate,year,month,(select name from branch_master where id=spr.branch_id )as subsidiary from special_auditorAdjustmentGL_rate spr order by id desc')->result();
		$month_array = $this->Master_Model->getQuarter();
		$tableRows = array();
		if (count($mbData) > 0) {
			$i=1;
