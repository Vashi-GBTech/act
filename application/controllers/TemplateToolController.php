...
//			$valueSignIN = $this->input->post('valueSignIN');
			...
			try {
				...
				if (!empty($template_id)) {
					$updatetemplateMaster = array('template_name' => $template_name, 'prefill' => $prefill, 'modify_by' => $user_id, 'modify_on' => date('Y-m-d H:i:s'));
...
from handson_template_column_master hm where template_id="' . $temp_id . '" order by sequence asc');
			...
			if ($columnObject->totalCount > 0) {
				...
				if($branch_id==136)
				{
					$rowsObject=$this->TemplateToolModel->_select('handson_transaction_table hm', array('template_id' => $temp_id, 'company_id' => $company_id,'year' => $year, 'month' => $month, 'status' => 1,'branch_id!='=>136),
				...
				if (count($alldataArray) > 0) {
					...
					foreach ($finalArray as $key1 => $rowR) {
						...
						foreach ($rowR as $r1) {
							...
							$html .= '<td style="font-size: 11px;text-align:right;width: 10%">' .round($r1, 2)  . '</td>';
						}
						$html .= '</tr>';
					}
					$html .= '</tbody>';
...
<td></td>
<td></td>
<td style='color: black;font-weight: bold'>" . array_sum($a_opening_bal) . "</td>
<td style='color: black;font-weight: bold'> " . array_sum($a_debit) . "</td>
<td style='color: black;font-weight: bold'>" . array_sum($a_credit) . "</td>
<td style='color: black;font-weight: bold'>" . array_sum($a_total) . "</td>
</tr>";
			$html .= "</tbody></table>";
		} else {
			$html = '';
