...
<script>
	...
	function downlodAllbranch(type) {
		...
		if((year == "") || (month=="")){
			...
			if(type==2)
			{
				...
				window.location.href=base_url+"DownloadAllScheduleForSubisdiary?month="+$("#month").val()+"&year="+$("#year").val()+"&temp_id="+temp_id+"&branch_id="+branch_id;
			}
			else {
				var base_url='<?= base_url() ?>';
				window.location.href=base_url+"DownloadAllScheduleForSubisdiary?month="+$("#month").val()+"&year="+$("#year").val()+"&temp_id="+temp_id;
			}
		}
	}
	function downlodAllbranchRupees(type) {
		...
		if((year == "") || (month=="")){
			...
			{
				var base_url='<?= base_url() ?>';
				window.location.href=base_url+"DownloadAllRupeesScheduleForSubsidiary?month="+$("#month").val()+"&year="+$("#year").val()+"&temp_id="+temp_id+"&branch_id="+branch_id;
			}
			else {
				var base_url='<?= base_url() ?>';
				window.location.href=base_url+"DownloadAllRupeesScheduleForSubsidiary?month="+$("#month").val()+"&year="+$("#year").val()+"&temp_id="+temp_id;
			}
		}
	}
	...
	function downlodRupeesConversionAllbranch() {
		...
		if(month!="" && month!=null && year!="" && year!=null && temp_id!="" && temp_id!=null) {
			console.log(temp_id);
			var base_url='<?= base_url() ?>';
			window.location.href=base_url+"downlodRupeesConversionAllbranch?month="+$("#month").val()+"&year="+$("#year").val()+"&temp_id="+temp_id;
		}
		else {
			toastr.error('Select month & year');
		}
